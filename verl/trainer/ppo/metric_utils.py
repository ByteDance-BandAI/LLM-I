...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    img_num_score = torch.Tensor(batch.non_tensor_batch['img_num_score'])
    llm_score = torch.Tensor(batch.non_tensor_batch['llm_score'])
    mllm_score = torch.Tensor(batch.non_tensor_batch['mllm_score'])
    llm_text_score = torch.Tensor(batch.non_tensor_batch['llm_text_score'])
    llm_image_score = torch.Tensor(batch.non_tensor_batch['llm_image_score'])
    img_quality_score = torch.Tensor(batch.non_tensor_batch['img_quality_score'])
    ti_rel_score = torch.Tensor(batch.non_tensor_batch['ti_rel_score'])
    tq_rel_score = torch.Tensor(batch.non_tensor_batch['tq_rel_score'])
    r_tool = torch.Tensor(batch.non_tensor_batch['r_tool'])
    score = torch.Tensor(batch.non_tensor_batch['score'])
    ...
    metrics = {
        ...
        **(
        ...
        # llmi score
        "score/img_num_score": torch.mean(img_num_score),
        "score/llm_score": torch.mean(llm_score),
        "score/mllm_score": mllm_score[img_quality_score != 0].mean(),
        "score/llm_text_score": torch.mean(llm_text_score),
        "score/llm_image_score": torch.mean(llm_image_score),
        "score/img_quality_score": img_quality_score[img_quality_score != 0].mean(),
        "score/ti_rel_score": ti_rel_score[ti_rel_score != 0].mean(),
        "score/tq_rel_score": tq_rel_score[tq_rel_score != 0].mean(),
        "score/r_tool": torch.mean(r_tool),
        "score/overall_score": torch.mean(score),
    }
    ...
    for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
        for prompt, var2vals in prompt2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                if var_name in {"ti_rel_score", "tq_rel_score", "img_quality_score"}:
                    var_vals = np.array(var_vals)
                    var_vals = var_vals[var_vals != 0].tolist()
