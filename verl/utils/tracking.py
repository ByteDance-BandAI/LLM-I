...
@dataclasses.dataclass
class LLMIGenerationLogger:
    def __init__(self, training: bool, sp_path: str = "system_prompts/training/llmi.txt"):
    ...
    def log_generations_to_wandb(self, samples, step):
        ...
        for i, sample in enumerate(samples):
            try:
                input_text, raw_output, scores, input_img = sample
            except:
                input_text, raw_output, scores = sample
                input_img = None
            ...
            if input_img:
                for img in input_img:
                    if img:
                        ...
                        img_tag = f'<img src="data:image/png;base64,{img_base64}" alt="Generated Image" style="max-width: 400px; display: block; margin: 8px 0;">'
                        input_img_tags.append(img_tag)

            input_question = (
                input_text.replace(self.sp, "")
                .replace("system\n", "")
                .replace("assistant\n", "")
                .strip()
            )
            ...
            if input_img_tags:
                input_question = "\t".join(input_img_tags) + "\n" + input_question

            input_question_html = markdown.markdown(
                input_question,
                extensions=["fenced_code", "tables", "sane_lists", "nl2br"],
            )

            vlm_output, pil_images, _ = (
                self.interleaved_generator.interleaved_generation_concurrent(raw_output)
            )
            ...
            markdown_with_placeholders = vlm_output
            for tag in img_tags:
            ...
            markdown_with_placeholders = markdown_with_placeholders.replace(
                "<fail_to_generate_image>", "`<fail_to_generate_image>`"
            )

            final_body_html = markdown.markdown(
            ...
            interleaved_html = wandb.Html(final_styled_html)

            row_data.extend([input_text, raw_output, interleaved_html, scores])

        new_table.add_data(*row_data)

        log_key = "train/generations" if self.training else "val/generations"
