...
def is_image_data_missing(value):
    try:
        is_na_result = pd.isna(value)
        if isinstance(is_na_result, np.ndarray):
            return np.all(is_na_result)
        return is_na_result
    except TypeError:
        return False
...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.max_image_resolution = config.get("max_image_resolution", 2048 * 28 * 28)
    ...
    def _read_files_and_tokenize(self):
        if self.allow_heterogeneous_schemas:
            ...
            if self.image_key in combined_df.columns:
                combined_df['modality'] = combined_df[self.image_key].apply(
                    lambda x: 'text' if is_image_data_missing(x) else 'multimodal'
                )
            else:
                combined_df['modality'] = 'text'
            
            combined_df.replace({np.nan: None}, inplace=True)  # replace NaN with None
            
            modality_counts = combined_df['modality'].value_counts()
            multimodal_count = modality_counts.get('multimodal', 0)
            text_count = modality_counts.get('text', 0)
            total_count = len(combined_df)

            print(f"  -> Number of multimodal data: {multimodal_count}")
            print(f"  -> Number of text data: {text_count}")
            print("-" * 35)
            print(f"  -> Total number of data: {total_count}")
            print("="*60)

            if total_count != (multimodal_count + text_count):
                print("\n[WARNING] Total number of data does not match the sum of multimodal and text data counts, please check the data loading or merging logic!\n")
    ...
    def maybe_resize_mm_data(self, dataframe: Union[datasets.Dataset, list[dict]] = None):
        ...
        def _add_pixel_example(example):
            ...
            modality = example.get('modality', None)
            if modality == 'multimodal' and imgs is not None:
                new_imgs = []
    ...
    def _build_messages(self, example: dict):
        if "modality" in example and example['modality'] == 'multimodal':
            messages: list = list(example.pop(self.prompt_key))
    ...
    def __getitem__(self, item):
        ...
        if self.replace_system_prompt:
            # Replace origin system prompt with new_sp
            if "modality" in row_dict:
                if row_dict['modality'] == 'multimodal':
                    num_sp = 0
                    for me in messages:
                        if me["role"] == "system":
                            num_sp += 1
                    if num_sp == 0:
                        ...
                        assert messages[0]["role"] == "system", "The first message must be system, when set replace_system_prompt to True"
                        messages[0]["content"] = self.new_sp
                        pass
                    elif num_sp > 1:
                        raise ValueError("More than one system message is found")
                else:
                    messages = [{"role": "system", "content": self.new_sp}, 
                                {"content": [{"type": "text", "text": messages}], "role": "user"}]
            else:
                messages = [{"role": "system", "content": self.new_sp}, 
        ...
        if self.processor is not None:
            ...
            images = None
            if self.image_key in row_dict and "modality" in row_dict:
                if row_dict['modality'] == 'multimodal':
                    ...
                    assert len(images) == messages[1]["content"].count({"type": "image"})
                else:
                    row_dict.pop(self.image_key)
                    images = [Image.new("RGB", (28, 28), (255, 255, 255))]
                    multi_modal_data["image"] = images
                    assert isinstance(messages[-1]["content"], list) and messages[-1]["role"] == "user"
                    messages[-1]["content"] = [{"type": "image"}] + messages[-1]["content"]
