...
PROJECT_NAME="LLMI"
EXP_SUFFIX="mm"
EXP_NAME="QwenVL-7B-${EXP_SUFFIX}"
SAVE_DIR="YOUR_SAVE_DIR/${PROJECT_NAME}/${EXP_NAME}"
...
# Data Path
train_files="['train.parquet', 'train_mm.parquet']"
test_files="['test.parquet', 'test_mm.parquet']"
...
train_micro_bsz_per_gpu=2
log_prob_micro_bsz_per_gpu=2

# multimodal input
enable_mm_resize=true
max_image_resolution=$((1024 * 1024))
allow_heterogeneous_schemas=true

# Judge
llm_text_factor=1.5
llm_image_factor=0.5
...
ray job submit --no-wait \
    ...
    +data.enable_mm_resize=${enable_mm_resize} \
    +data.max_image_resolution=${max_image_resolution} \
    +data.allow_heterogeneous_schemas=${allow_heterogeneous_schemas} \
